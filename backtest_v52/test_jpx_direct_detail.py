...
links=[a for a in ss.find_all('a',href=True) if 'ixbrl' in a['href']]
print(d.status_code,len(d.text),ss.title.get_text(' ',strip=True) if ss.title else '')
print('ixbrl',len(links))
if not links:
    raise RuntimeError('Direct JPX issuer-detail access returned no iXBRL links')
