...
def fetch_detail(code:int):
    session=requests.Session(); session.headers.update({"User-Agent":"Mozilla/5.0 (compatible; v6-research-audit/1.1)"})
    r=session.get(SEARCH_URL,timeout=45); r.raise_for_status(); r.encoding="utf-8"
    s=BeautifulSoup(r.text,"html.parser"); f=s.find("form")
    if not f: raise RuntimeError("JPX search form not found")
    payload=hidden_inputs(f)
    payload.update({"eqMgrCd":str(code),"mgrMiTxtBx":"","dspSsuPd":"200","ListShow":"ListShow","jjHisiKbnChkbx":"on"})
    for k in ["Show","Switch"]: payload.pop(k,None)
    rr=session.post(urljoin(r.url,f.get("action")),data=payload,timeout=45); rr.raise_for_status(); rr.encoding="utf-8"
    ss=BeautifulSoup(rr.text,"html.parser"); rf=ss.find("form")
    ...
    for b in rf.find_all("input",attrs={"type":"button"}):
        ...
        if m and m.group(1).startswith(str(code)): target=(m.group(1),m.group(2)); break
    ...
    d=session.post(urljoin(rr.url,rf.get("action")),data=rd,timeout=60); d.raise_for_status(); d.encoding="utf-8"; ds=BeautifulSoup(d.text,"html.parser")
    meta={}
    for name in ["eqMgrNm","szkbuNm","gyshDspNm"]:
        x=rf.find("input",attrs={"name":re.compile(re.escape(name)+r"$")}); meta[name]=x.get("value","") if x else ""
    meta.update({"jpx_code5":jpx_code5,"detail_url":d.url}); return ds,meta
...
def extract_rows(ds,code,meta,start,end):
    ...
    for tr in ds.find_all("tr",id=ROW_ID_RE):
        ...
        pdf_url=urljoin(DISC_BASE,title_link["href"]) if title_link and title_link.get("href","").lower().endswith(".pdf") else ""
        ixbrl=qualitative=xbrl_zip=""
        ...
        seen.add(key)
        rows.append(DisclosureRow(code,meta.get("jpx_code5",""),meta.get("eqMgrNm",""),meta.get("szkbuNm",""),meta.get("gyshDspNm",""),dt.isoformat(),CATEGORY.get(cat_code,cat_code),title,pdf_url,ixbrl,qualitative,xbrl_zip,meta.get("detail_url",""),fetched))
    return rows
...
def one(code,start,end,retries):
    ...
    for attempt in range(retries+1):
        try:
            ...
            err=repr(e); time.sleep(0.5*(attempt+1))
    return [],err
...
def main():
    ...
    all_rows.sort(key=lambda r:(r.code,r.disclosure_date,r.category,r.title))
    args.out.parent.mkdir(parents=True,exist_ok=True); fields=list(DisclosureRow.__annotations__.keys())
    with args.out.open("w",newline="",encoding="utf-8-sig") as f:
        w=csv.DictWriter(f,fieldnames=fields); w.writeheader(); [w.writerow(asdict(r)) for r in all_rows]
