...
import argparse, csv, io, re, subprocess, tempfile
...
UA={"User-Agent":"Mozilla/5.0 (compatible; v6-research-audit/1.0)"}

PROFIT_KEYS=[
    "OperatingIncomeIFRS","OperatingIncome",
    "OrdinaryIncome","ProfitBeforeTaxIFRS","ProfitBeforeTax",
    "ProfitAttributableToOwnersOfParentIFRS","ProfitAttributableToOwnersOfParent","NetIncome"
]
...
def pick_forecast(facts,keys):
    ...
    for name,ctx,x,raw in facts:
        ...
        candidates.append((score,x,name,ctx))
    return max(candidates,default=(None,None,None,None),key=lambda z:z[0])[1]

def pick_change(facts):
    candidates=[]
    for name,ctx,x,raw in facts:
        if x is None or name not in CHANGE_KEYS:continue
        if "ResultMember" not in ctx:continue
...
def pick_dividend(facts):
    vals=[]
    for name,ctx,x,raw in facts:
        if name!=DIV_KEY or x is None:continue
        if "AnnualMember" not in ctx or "ForecastMember" not in ctx:continue
        if "LowerMember" in ctx or "UpperMember" in ctx:continue
        score=3 if "NextYearDuration" in ctx else 2 if "CurrentYearDuration" in ctx else 1
        vals.append((score,x))
...
def detect_dilution(rows):
    for _,r in rows.iterrows():
        t=str(r.get("title",r.get("Title","")))
        if any(k in t for k in ["第三者割当による新株式","公募増資","新株式発行","転換社債型新株予約権付社債"]):
            return 1
    return 0

def semantic_points(rows):
    # Strict deterministic extraction. No score unless both a quantitative target/effect and a concrete action are explicit in primary PDF text.
    p2c=p2d=p1b=gap=0
    texts=[]
    for _,r in rows.iterrows():
        title=str(r.get("title",r.get("Title","")))
    ...
    alltxt="\n".join(texts)
    if alltxt:
        quant_cap=bool(re.search(r"(?:ROE|ROIC|FCF|フリー.?キャッシュ.?フロー)[^\n]{0,80}?(?:[0-9]+(?:\.[0-9]+)?\s*[%％]|[0-9,]+\s*億円)",alltxt,re.I))
        action_cap=any(k in alltxt for k in ["政策保有株式","売却","撤退","事業ポートフォリオ","資産圧縮","投資配分","不採算事業"])
        p2c=2 if quant_cap and action_cap else 0
        restruct=any(k in alltxt for k in ["構造改革","事業再編","固定費削減","コスト削減"])
        quant_effect=bool(re.search(r"(?:営業利益|事業利益|FCF|フリー.?キャッシュ.?フロー)[^\n]{0,100}?(?:改善|効果|削減)[^\n]{0,80}?[0-9,]+\s*億円",alltxt))
        p2d=2 if restruct and quant_effect else 0
        # KPI point is intentionally conservative: require explicit Book-to-Bill >1.05.
        m=re.search(r"Book[- ]?to[- ]?Bill[^0-9]{0,30}([0-9]+(?:\.[0-9]+)?)",alltxt,re.I)
        if m and float(m.group(1))>1.05: p1b=1
    return p2c,p2d,p1b,gap

def score_code(code,df):
    df=df.sort_values(["disclosure_date","category","title"]).copy()
    fin=df[(df["category"]=="financial_results") & df["ixbrl_url"].fillna("").ne("")].copy()
    facts_cache={}
    for idx,r in fin.iterrows():
        try:facts_cache[idx]=ix_facts(r["ixbrl_url"])
        except Exception as e:facts_cache[idx]=[]
    prev_fc=None;prev_div=None;events=[]
    for dt,day in df.groupby("disclosure_date",sort=True):
        fday=fin[fin["disclosure_date"]==dt]
        current_facts=[]
        if not fday.empty:
            current_facts=facts_cache.get(fday.index[-1],[])
        fc=pick_forecast(current_facts,PROFIT_KEYS+EPS_KEYS) if current_facts else None
        yoy=pick_change(current_facts) if current_facts else None
        div=pick_dividend(current_facts) if current_facts else None
        fchg=(fc/prev_fc-1)*100 if fc is not None and prev_fc not in (None,0) else None
        dchg=(div/prev_div-1)*100 if div is not None and prev_div not in (None,0) else None
        p4=4 if fchg is not None and fchg>=5 else 0
        m5=5 if fchg is not None and fchg<=-5 else 0
        p2a=2 if yoy is not None and yoy>=15 and (fchg is None or fchg>=-0.01) else 0
        m2a=2 if yoy is not None and yoy<=-20 else 0
        p1a=1 if dchg is not None and dchg>=10 else 0
        bp=buyback_pct(day); p2b=2 if bp is not None and bp>=1 and not detect_dilution(day) else 0
        m3=3 if detect_dilution(day) else 0
        p2c,p2d,p1b,gap=semantic_points(day)
        titles=" | ".join(day["title"].astype(str).tolist())
        if any(k in titles for k in ["継続企業の前提","監査意見","不適切会計","粉飾","訂正有価証券報告書"]):m5=max(m5,5)
        score=p4+p2a+p2b+p1a+p2c+p2d+p1b-m5-m3-m2a-gap
        primary=1 if any([p4,p2a,p2b,p2c,p2d]) else 0
        passed=int(score>=4 and m5==0 and primary==1)
        urls=" | ".join([u for u in day["pdf_url"].fillna("").astype(str).tolist()+day["ixbrl_url"].fillna("").astype(str).tolist() if u])
        events.append(EventScore(int(code),dt,int(score),passed,int(m5>0),p4,p2a,p2b,p1a,p2c,p2d,p1b,m5,m3,m2a,gap,yoy,fchg,dchg,bp,primary,titles,urls,"C/D/KPI strict rule-based primary-text extraction; NovelText/ownership are ranking-only and calculated later"))
        if fc is not None:prev_fc=fc
        if div is not None:prev_div=div
    return events

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--disclosures",type=Path,required=True);ap.add_argument("--out",type=Path,required=True);ap.add_argument("--workers",type=int,default=8)
    args=ap.parse_args();df=pd.read_csv(args.disclosures)
    if df.empty:
        args.out.parent.mkdir(parents=True,exist_ok=True);pd.DataFrame(columns=list(EventScore.__annotations__)).to_csv(args.out,index=False);return
    ...
    with args.out.open("w",newline="",encoding="utf-8-sig") as f:
        w=csv.DictWriter(f,fieldnames=list(EventScore.__annotations__.keys()));w.writeheader();[w.writerow(asdict(x)) for x in all_events]
